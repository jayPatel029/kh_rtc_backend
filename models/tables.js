  ...
  try {
    const query = `
      CREATE TABLE IF NOT EXISTS tele_doctor (
        ...
        onVacation TINYINT(1) NULL DEFAULT 0,
...
const createPatientClinicTable = async () => {
  try {
    const query = `
      CREATE TABLE IF NOT EXISTS tele_patient_clinic (
    ...
    `;
    await sequelize.query(query);
    console.log("tele_patient_clinic table created successfully");
  } catch (error) {
    console.error("Error creating tele_patient_clinic table:", error);
  }
};
...
const createTeleChatTable = async () => {
  try {
    const query = `
CREATE TABLE IF NOT EXISTS tele_chat_messages (
  id INT AUTO_INCREMENT PRIMARY KEY,
  sender_id INT NOT NULL,
  sender_type ENUM('admin', 'doctor', 'front_desk') NOT NULL,
  receiver_id INT NOT NULL,
  receiver_type ENUM('admin', 'doctor', 'front_desk') NOT NULL,
...
`;
    await sequelize.query(query);
    console.log("tele_chat_messages table created successfully");
  } catch (error) {
...
const createAllTables = async () => {
  try {
    ...
    await createPatientClinicTable();
